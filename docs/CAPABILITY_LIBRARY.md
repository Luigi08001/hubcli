...
- Email subscription type creation is possible through `hscli communication-preferences definitions create-internal` using browser-session/internal `/api/subscriptions/v1/definitions?portalId=<portalId>`; this is treated as an internal migration adapter with portal guards, composite idempotency, BU remapping, and an Email-only channel guard.
